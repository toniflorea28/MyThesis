<template>
  ...
  >
    ...
    <div class="text-center">
      <h1 class="mt-5">
        Planuri de abonamente
      </h1>
      <p class="mb-2 pb-75">
        Toate planurile includ uneltele de care ai nevoie ca să îți duci planurile la bun sfărșit.
      </p>
        ...
        <b-row>
            ...
            >
              ...
              <div class="annual-plan">
                <div class="plan-price mt-2">
                  ...
                  <sub class="pricing-duration text-body font-medium-1 font-weight-bold">/ lunar</sub>
                </div>
                <small
                  v-show="!monthlyPlanShow"
                  class="annual-pricing text-muted"
                >USD {{ pricing.basicPlan.yearlyPlan.totalAnual }} / anual</small>
              </div>
              <!--/ annual plan -->
            ...
            >
              ...
              <div class="annual-plan">
                <div class="plan-price mt-2">
                  ...
                  <sub class="pricing-duration text-body font-medium-1 font-weight-bold">/ anual</sub>
                </div>
                <small
                  v-show="!monthlyPlanShow"
                  class="annual-pricing text-muted"
                >USD {{ pricing.standardPlan.yearlyPlan.totalAnual }} / anual</small>
              </div>
              <!--/ annual plan -->
          ...
          <b-col md="4">
            ...
            >
              <!-- img -->
              <b-img
                v-if="pricing.vipPlan.img"
                :src="require('../../../assets/images/illustration/Pot3.svg')"
                class="mb-2"
                alt="enterprise svg img"
              />
              <!--/ img -->
              <h3>{{ pricing.vipPlan.title }}</h3>
              <b-card-text>{{ pricing.vipPlan.subtitle }}</b-card-text>

              <!-- annual plan -->
              <div class="annual-plan">
                <div class="plan-price mt-2">
                  <sup class="font-medium-1 font-weight-bold text-primary">$</sup>
                  <span class="pricing-basic-value font-weight-bolder text-primary">{{ monthlyPlanShow ? pricing.vipPlan.monthlyPrice:pricing.vipPlan.yearlyPlan.perMonth }}</span>
                  <sub class="pricing-duration text-body font-medium-1 font-weight-bold">/ lunar</sub>
                </div>
                <small
                  v-show="!monthlyPlanShow"
                  class="annual-pricing text-muted"
                >USD {{ pricing.vipPlan.yearlyPlan.totalAnual }} / anual</small>
              </div>
              <!--/ annual plan -->

              <!-- plan benefit -->
              <b-list-group
                v-for="(data,index) in pricing.vipPlan.planBenefits"
                :key="index"
          ...
          <div class="pricing-trial-content d-flex justify-content-between">
            <div class="text-center text-md-left mt-3">
              <h3 class="text-primary">
                Nu ești convins? Încearcă un abonament standard timp de 14 zile.
              </h3>
              <h5>Vei avea acces complet la toate funcțiile timp de 14 zile.</h5>
              <b-button
                v-ripple.400="'rgba(255, 255, 255, 0.15)'"
                variant="primary"
                class="mt-2 mt-lg-3"
...
export default {
  ...
  created() {
    this.$http.get('/api/pricing/data').then(res => {
      this.pricing = res.data
      this.pricing.basicPlan.yearlyPlan = JSON.parse(this.pricing.basicPlan.yearlyPlan)
      this.pricing.standardPlan.yearlyPlan = JSON.parse(this.pricing.standardPlan.yearlyPlan)
      this.pricing.vipPlan.yearlyPlan = JSON.parse(this.pricing.vipPlan.yearlyPlan)
    })
  },
  methods: {
    tooglePlan() {
